...
# CORS middleware (helps frontend talk to backend)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Open for testing; tighten in production
    allow_methods=["*"],
    allow_headers=["*"],
)
...
def check_model(model_name: str):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error checking model: {e}")
    ...
    try:
        with requests.post(f"{OLLAMA_URL}/api/pull", json={"name": model_name}, stream=True) as r:
            for line in r.iter_lines():
                if line:
                    pass  # You could stream progress here if needed
        return {"success": True}
    except Exception as e:
